...
export default function BusinessSection() {
  ...
  return (
    <section id="business-pillars" className="py-20 bg-white border-b border-slate-100">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {businessPillars.map((pillar) => (
            <div
              key={pillar.id}
              id={pillar.id}
              className="group bg-slate-50 rounded-3xl border border-slate-200/60 overflow-hidden shadow-sm hover:shadow-xl hover:border-slate-300/80 transition-all duration-300 flex flex-col"
            >
              ...
              <div className="relative h-56 sm:h-64 overflow-hidden bg-slate-900 shrink-0">
                <img
                  ...
                  className="w-full h-full object-cover transform group-hover:scale-105 transition-all duration-700 ease-out opacity-90 group-hover:opacity-100"
                />
                {/* Visual Accent Overlay */}
                <div className="absolute inset-0 bg-gradient-to-t from-slate-950/40 via-transparent to-transparent"></div>
                <div className="absolute top-4 left-4 bg-white/95 backdrop-blur p-3 rounded-2xl shadow-md border border-slate-100">
                  {pillar.icon}
                </div>
              </div>
              ...
              <div className="p-6 sm:p-8 flex-1 flex flex-col justify-between space-y-6">
                <div className="space-y-3">
                  ...
                  <h3 className="text-xl font-bold text-slate-900 leading-snug group-hover:text-orange-600 transition-colors">
                    {pillar.title}
                  </h3>
                ...
                <div className="space-y-2 pt-4 border-t border-slate-200/60">
                  {pillar.features.map((feature, i) => (
                    <div key={i} className="flex items-center space-x-2 text-xs sm:text-sm text-slate-600">
                      <div className="w-1.5 h-1.5 bg-orange-600 rounded-full shrink-0"></div>
                      <span>{feature}</span>
                    </div>
                  ))}
                </div>
