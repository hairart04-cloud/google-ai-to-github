...
export default function ReservationSection({ user, onOpenAuth }: ReservationSectionProps) {
  ...
  return (
    <section id="reservation" className="py-24 bg-white text-slate-800 relative border-b border-slate-200">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,rgba(249,115,22,0.03),transparent_35%)]"></div>
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
              <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-6">
                <div className="flex items-center space-x-2.5">
                  <CalendarIcon className="w-5.5 h-5.5 text-orange-600" />
                  <span className="text-base font-bold text-slate-900">{currentMonthName} 달력</span>
                </div>
                <span className="text-xs text-slate-400 font-mono">* 오늘 날짜: 7월 5일</span>
              </div>

              {/* Day of Week Headers */}
              <div className="grid grid-cols-7 gap-2 text-center text-xs font-bold uppercase tracking-wider text-slate-400 mb-3 font-mono">
                    ...
                    <button
                      ...
                      className={`aspect-square rounded-xl text-xs sm:text-sm font-semibold flex flex-col items-center justify-center transition-all relative cursor-pointer ${
                        isPast
                          ...
                          : isSelected
                          ? 'bg-orange-650 text-white shadow-md shadow-orange-600/20 ring-2 ring-orange-500/10 scale-105'
                          : 'bg-slate-50 hover:bg-slate-100 text-slate-800 border border-slate-150'
                      }`}
                    >
                      <span className={isPast ? '' : isSunday ? 'text-red-500' : isSaturday ? 'text-blue-500' : ''}>
                        {day}
                      </span>
                      {day === 5 && (
                        <span className="absolute bottom-1.5 w-1 h-1 rounded-full bg-orange-500"></span>
                      )}
                    </button>
                  );
                })}
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
              <div className="flex items-center space-x-2.5 border-b border-slate-100 pb-4 mb-6">
                ...
                <span className="text-base font-bold text-slate-900">희망 방문 시간 선택</span>
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={`py-3 px-1 rounded-xl text-xs sm:text-sm font-bold tracking-wider font-mono text-center transition-all cursor-pointer border ${
                        isSelected
                          ? 'bg-orange-650 text-white border-orange-600 shadow-sm scale-105'
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-sm h-full">
              {submitted ? (
                <div className="text-center py-16 space-y-6 animate-fade-in flex flex-col items-center justify-center h-full">
                  ...
                  <h3 className="text-2xl font-bold text-slate-900">방문 예약 신청 완료!</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <h3 className="text-lg font-bold border-b border-slate-100 pb-4 flex items-center gap-2 text-slate-900">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className="block text-xs font-semibold uppercase text-slate-500 mb-1.5">방문 상담 유형</label>
                            ...
                            <button
                              ...
                              className={`py-2 px-1 text-xs rounded-lg font-bold transition-colors cursor-pointer border ${
                                reservationType === type
                                  ? 'bg-orange-650 text-white border-orange-600 shadow-sm'
                    ...
                    <div>
                      <label className="block text-xs font-semibold uppercase text-slate-500 mb-1.5">예약자 연락처</label>
                    ...
                    <div>
                      <label className="block text-xs font-semibold uppercase text-slate-500 mb-1.5">상담 및 전달 내용</label>
                      <textarea
                        ...
                        className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-4 text-sm text-slate-800 focus:outline-none focus:border-orange-500 transition-colors resize-none"
                      />
                    </div>
                  </div>

                  {!user && (
                    <div className="bg-orange-50 border border-orange-100 rounded-xl p-3.5 flex items-start gap-2 text-xs text-orange-800">
                      <ShieldAlert className="w-4 h-4 shrink-0 mt-0.5 text-orange-600" />
                  ...
                  <button
                    ...
                    type="submit"
                    disabled={loading}
                    className="w-full py-3.5 bg-orange-600 hover:bg-orange-700 disabled:bg-orange-800 text-white font-bold rounded-xl transition-colors cursor-pointer text-sm"
                  >
