...
export default function FloatingChatbot() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      ...
      {showTooltip && !isOpen && (
        <div className="mb-3 mr-2 bg-navy-950 text-white text-xs sm:text-sm py-2.5 px-4 rounded-2xl shadow-xl border border-navy-800 animate-fade-in max-w-[260px] relative">
          <button 
          ...
          <div className="absolute right-4 -bottom-1.5 w-3 h-3 bg-navy-950 rotate-45 border-r border-b border-navy-800"></div>
        </div>
      )}
      ...
      <button
        ...
        className={`p-4 rounded-full shadow-2xl transition-all transform hover:-translate-y-1 hover:scale-105 cursor-pointer flex items-center justify-center ${
          isOpen 
            ? 'bg-navy-950 ring-4 ring-navy-950/10 text-gold-500 border border-navy-800' 
            : 'bg-gradient-to-tr from-gold-500 to-gold-400 text-navy-950 shadow-gold-500/35 hover:shadow-gold-500/45 ring-4 ring-gold-500/10'
        }`}
      ...
      {isOpen && (
        <div 
          ...
          className="absolute bottom-18 right-0 w-[90vw] sm:w-[400px] h-[550px] bg-navy-950 border border-navy-800 rounded-3xl shadow-2xl flex flex-col justify-between overflow-hidden animate-fade-in-up"
        >
          {/* Header */}
          <div className="bg-navy-950 p-4 sm:p-5 text-white border-b border-navy-850 flex items-center justify-between shrink-0">
            <div className="flex items-center space-x-3">
              <div className="p-2 bg-gradient-to-tr from-gold-500 to-gold-400 rounded-xl text-navy-950">
                <Sparkles className="w-5 h-5" />
              </div>
              <div>
                <h3 className="text-sm font-black tracking-wide">LAVACORE AI 스마트 비서</h3>
                <p className="text-[10px] text-gold-400 font-mono flex items-center gap-1 mt-0.5">
            ...
            <button
              ...
              className="p-1 rounded-lg hover:bg-navy-900 text-slate-400 hover:text-white transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          {/* Chat Messages */}
          <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 bg-navy-900 custom-scrollbar">
            {messages.map((msg) => (
              <div
                key={msg.id}
              ...
              >
                <div
                  className={`max-w-[85%] rounded-2xl px-4 py-3 text-xs sm:text-sm leading-relaxed shadow-sm whitespace-pre-line ${
                    msg.sender === 'user'
                      ? 'bg-gold-500 text-navy-950 font-semibold rounded-tr-none'
                      : 'bg-navy-950 text-slate-200 border border-navy-850 rounded-tl-none'
                  }`}
                >
                  {msg.text}
                </div>
            ...
            {isTyping && (
              <div className="flex justify-start">
                <div className="bg-navy-950 border border-navy-850 rounded-2xl rounded-tl-none px-4 py-3 text-xs sm:text-sm leading-relaxed shadow-sm flex items-center space-x-1.5 text-slate-400">
                  <span className="w-1.5 h-1.5 rounded-full bg-gold-500/60 animate-bounce"></span>
                  <span className="w-1.5 h-1.5 rounded-full bg-gold-500/60 animate-bounce [animation-delay:0.2s]"></span>
                  <span className="w-1.5 h-1.5 rounded-full bg-gold-500/60 animate-bounce [animation-delay:0.4s]"></span>
                </div>
              </div>
            )}
          ...
          <div className="px-4 sm:px-5 py-3 border-t border-navy-850 bg-navy-950 shrink-0">
            ...
            <div className="grid grid-cols-2 gap-2">
              {quickQuestions.map((q, idx) => (
                <button
                  ...
                  className="text-[11px] bg-navy-900 hover:bg-navy-850 hover:text-white border border-navy-850 hover:border-gold-500/50 rounded-xl px-2.5 py-2 text-slate-300 transition-colors text-left font-medium cursor-pointer"
                >
                  {q.label}
                </button>
              ))}
          ...
          {/* Text Input Form */}
          <div className="p-4 border-t border-navy-850 bg-navy-950 shrink-0 flex items-center gap-2">
            <input
            ...
            <button
              ...
              className="p-2.5 rounded-xl bg-gold-500 hover:bg-gold-600 text-navy-950 font-bold transition-colors cursor-pointer"
            >
